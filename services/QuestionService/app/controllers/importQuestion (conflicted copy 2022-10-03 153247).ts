  ...
  if (fileName[1] === 'csv') {
    await new Promise((resolve, _reject) => {
      ...
      parser.on('error', function () {
        throw new HttpNotFound( "csv parse process failed");
        // reject(parseMsg)
      })
    })
  } else {
    await new Promise((resolve, _reject) => {
      ...
      file.on('error', function () {
        throw new HttpNotFound("xlsx parse process failed");
        // reject(parseMsg);
      })
    })
  }
