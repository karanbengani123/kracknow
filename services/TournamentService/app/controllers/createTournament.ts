...
export const addTournament = async (
  ...
  let id = '00000';
  console.log("Inputs Structure:", JSON.stringify(inputs, null, 2));
  ...
  try {
    ...
    console.log("Tournament Entity:", JSON.stringify(tournamentEntity, null, 2));
    
    // Create tournament and handle associated models
    await Tournament.create(tournamentEntity, {
      include: [
        { model: TournamentCity, as: "tournamentCities" },
        { model: TournamentKeyword, as: "tournamentKeywords" },
        { model: TournamentExamSchedule, as: "tournamentExamSchedule" },
        { model: TournamentPriceRatio, as: "tournamentPrize" },
        { model: TournamentBanner, as: "banner" },
        { model: TournamentRankingFactor, as: "tournamentRankingFactor" },
      ],
      transaction,
    });

    // Update last tournament record if exists
    if (tournament) {
      await tournament.update({ isLastRecord: false }, { transaction });
    }
    ...
    console.error("Error constructing or saving tournamentEntity:", error);
