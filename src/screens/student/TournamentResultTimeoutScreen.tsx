...
import { GetExamDetailsAPI, GetMarksAPI, GetScheduleListAPI, GetTournamentMarksAPI, } from '../../api-services/User.api';
...
export default class TournamentResultTimeoutScreen extends React.Component<Props, State> {
    backHandler: any;
    ...
    _getMarksHandler(): void {
        // debugger;
        GetTournamentMarksAPI(this.props.route.params?.examUUID, this.props.route.params?.participationUUID)
            .then(response => {
                const statusCode = response.status;
                const data = statusCode === 200 ? response.json() : [];
            ...
            .then((res: {
                ...
                console.log("result paylod*********---", res.data.payload)
        ...
        return (
            <ScrollView showsVerticalScrollIndicator={false} >
                ...
                <View style={{ alignItems: 'center', marginTop: 40 }}>
                        <Text style={{ color: 'red' }}>Exam time is over</Text>
                    </View>
                    <View style={{ alignItems: 'center', marginTop: 40 }}>
                    ...
                    <View style={{ justifyContent: 'space-between', display: 'flex', marginTop: 100 }}>
                        ...
                        <View style={{ flexDirection: 'row', display: 'flex', justifyContent: 'space-around', marginBottom: 50 }}>
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <Text style={{ fontWeight: '500', fontSize: 14, color: '#0A1042', }}>Correct: {this.state.result.correct}</Text>
                            </View>
                            <View style={{ flexDirection: 'row' }}>
                                <Image style={{ height: 20, width: 20, marginRight: 5 }} source={require("../../../assets/images/remove.png")}></Image>
                                <Text style={{ fontWeight: '500', fontSize: 14, color: '#0A1042', }}>Wrong: {this.state.result.incorrect}</Text>
                            </View>
