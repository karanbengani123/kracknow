...
export default class TournamentLeaderboardScoreScreen extends React.Component<Props, State> {
  ...
  _getLeaderboardHandler(): void {
    // debugger;
    GetTournamentLeaderboardAPI(this.props.route.params?.uuid)
      .then((response) => {
        const statusCode = response.status;
        const data = statusCode === 200 ? response.json() : [];
      ...
      .then(
        (res: {
          ...
          // debugger;
          let rankSort = res.data.payload.response.leaderBoard.tournamentSchedule[0].tournamentPriceDistribution.sort(
          ...
          console.log(this.state.examLeaderboard, "<------------tour leaderboard")
        }
      );
  }
...

                {
                  this.state.examLeaderboard.map(data => (
                    <DataTable.Row>
                      ...
                      <DataTable.Cell style={{ flex: 1.5, }}>{data.name}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 1, }}>{data.marks}</DataTable.Cell>
                      <DataTable.Cell style={{ flex: 1, }}>{data.amount}</DataTable.Cell>
                    </DataTable.Row>
                  ))
                }
