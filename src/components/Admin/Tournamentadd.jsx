...
const Tournamentadd = () => {
    ...
    const AddTournament = () => {
        ...
        fetch(`${Environment.server_url}/tournaments/addTournament`, {
            method: "POST",
            body: description ?
                JSON.stringify({ categoryUUID, webBanner, phoneBanner, title, description, studentLimit, isFree, joinFee, joinDelay, tournamentKeywords, winningPrice, isFeatured, tournamentCities, tournamentExams, tournamentPrize: dataBundle, marksPerQuestion, allowPrimarySelection, allowSecondarySelection, tournamentRankingFactor: dataBundle1,timePerQuestion })
                :
                JSON.stringify({ categoryUUID, webBanner, phoneBanner, title, categoryUUID, allowPrimarySelection, allowSecondarySelection, isFeatured, marksPerQuestion, studentLimit, isFree, joinFee, marksPerQuestion, timePerQuestion, winningPrice, joinDelay, tournamentKeywords, tournamentCities, tournamentExams, tournamentPrize: dataBundle, tournamentRankingFactor: dataBundle1 }),
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`
            }
