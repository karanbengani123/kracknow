...
function Examview() {
    ...
    const [takeStartTime, setTakeStartTime] = useState("");
    const [takeEndTime, setTakeEndTime] = useState("");
    const [takeExamTime, setTakeExamTime] = useState("");
    ...
    const updateschedule = (startTime, endTime, examTime, uuid) => {
        // console.warn(moment(startTime).local().format('YYYY-MM-DD hh:mm'),"Start Time",moment(endTime).local().format('YYYY-MM-DD hh:mm'),"EndTime",examTime,"ExamTime",uuid,"ItemUUID")
    ...
    const ScheduleExam = async () => {
        ...
        let result = await fetch(`${Environment.server_url}/exams/schedule/${params.uuid}`,
            {
                method: "POST",
                body: JSON.stringify({ startTime, endTime, examTime, timeZone, type: "EXAM" }),
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`
                }
    ...
    const UpdateScheduleExam = async (uuid) => {
        // console.warn(moment(startTime),"StartTime")
        // console.warn(moment(endTime),"EndTime")
        // console.warn(examTime,"ExamTime")
                            ...
                            <div className="row">
                                <div className="col-12">
                                    <div className="page-title-box d-sm-flex align-items-center justify-content-between">
                                        <h4 className="mb-sm-0">Exam View</h4>
                                        {/* <ToastContainer /> */}
                                        <div className="page-title-right">
                                            <ol className="breadcrumb m-0">
                                                <Link to="/ExamNew" className="breadcrumb-item">Exam</Link>
                                                <li className="breadcrumb-item active">Exam View</li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
                                                                                ...
                                                                                studentDetails.map((item, index) =>
                                                                                    <tr>
                                                                                        ...
                                                                                        <td>
                                                                                            ...
                                                                                            {/* <span className="badge bg-orange mr-1">Not Attempted:<span>{item.notAttemptedCount}</span></span> */}
                                                                                        </td>
                                                                                        <td>
                                                                        ...
                                                                        getEndTime.map((item, key) =>
                                                                            item.schedule.map((obj, key) =>
                                                                                <tr key={key}>
                                                                                    ...
                                                                                    <td>
                                                                                        <span type="button" className="me-2 text-primary" data-bs-container="#tooltip-container1" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit" onClick={() => updateschedule(obj.startTime, obj.endTime, obj.examTime, obj.uuid)}><i className="mdi mdi-pencil font-size-18" />
                                                                                        </span>
