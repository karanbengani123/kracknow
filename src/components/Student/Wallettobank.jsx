...
function Wallettobank() {
  ...
  const [transactionListfull, setTransactionListfull] = useState([]);
  ...
  let walletballance = async (page) => {
    let result = await fetch(
      `http://localhost:3000/wallet/wallettransactionsforWithdrawal?limit=${itemsPerPage}&page=${page}`,
      {
        method: "GET",
        headers: {
    ...
    if (result.status == 200) {
      ...
      setTotalPage(
        Math.ceil(data.payload.Wallettransactionslistreq.count / itemsPerPage)
      );
      setTotalcount(data.payload.Wallettransactionslistreq.count);
    } else {
                    ...
                    <div className="card-body">
                      ...
                      <div className="table-responsive mt-3">
                        ...
                        >
                          ...
                          <tbody>
                            {addmoneyRequestList.map((resu) => {
                              return (
                                <tr className="hide-table-padding">
                                  <td>{getDateTime(resu.createdAt)}</td>
                                  <td>{resu.transferMode}</td>
                                  <td>{resu.amount}</td>
